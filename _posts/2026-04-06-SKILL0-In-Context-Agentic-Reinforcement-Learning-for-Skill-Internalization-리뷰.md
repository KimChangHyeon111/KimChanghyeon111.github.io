...
## 3. 제안 방법의 구조와 설계 의도: ICRL과 Dynamic Curriculum
SKILL0는 **In-Context Reinforcement Learning (ICRL)**과 **Dynamic Curriculum**을 결합하여 기술 내재화를 구현합니다.

### 3.1 In-Context Reinforcement Learning (ICRL)
ICRL은 SKILL0의 심장부와 같은 메커니즘입니다. 단순히 기술을 프롬프트에 넣는 것이 아니라, **강화학습의 탐색 능력과 기술 프롬프팅의 유도 편향(Inductive Bias)을 결합**합니다.

*   **동작 원리**: 에이전트는 훈련 중 기술 문맥(In-context skills)을 가이드로 활용하여 행동을 생성합니다. 이때 RL 알고리즘은 단순히 과업 성공 여부뿐만 아니라, **문맥 압축 효율성**까지 동시에 최적화합니다.
*   **Composite Reward (복합 보상)**: 논문에서는 다음과 같은 복합 보상 함수를 제안합니다.
    > $r_{t}^{comp} = \ln(c_t)$ (과업 성공 시), $0$ (실패 시)
    > $\tilde{r}_t = r_t + \lambda \cdot r_{t}^{comp}$
    여기서 $c_t$는 압축률을 의미하며, 모델이 최소한의 문맥 정보만으로도 과업을 성공했을 때 더 높은 보상을 주어 지식의 내재화를 강력하게 유도합니다.

### 3.2 Dynamic Curriculum (동적 커리큘럼)
